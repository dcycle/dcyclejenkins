...
docker-compose exec jenkins /bin/bash -c "sed -i.bak 's/<passwordHash>.*<\/passwordHash>/<passwordHash>$SALT:$HASH<\/passwordHash>/g' /var/jenkins_home/users/admin_*/config.xml"
